...
std::vector<int> longestpath(char *arr){ // works with DFS
	...
	_longestpath_check(0,0, currentpath, checked,result, arr); // coordinates, coordinates, currentpath, checked squares, the biggest way,
	return result;
}
// 3 problems
// comeback
// find longest way -> compare
// white squares

//void _find_island_check(int y, int x, std::array<bool, N*N> &checked, std::vector<int> &result, char *arr){
...
int main(){
		...
		sf::Texture image6;
		image6.loadFromFile("greenpurple.png");
